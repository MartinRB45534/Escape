...
class Monstre:
    ...
    def joueur_accesible(self,position_lab,vue,position_joueur):
        ...
        if self.joueur_en_vue(position_lab,vue,position_joueur):
            solution= Resolveur(vue,len(vue),len(vue[0]),position_joueur[0],position_joueur[1],self.position[0]-position_lab[0],self.position[1]-position_lab[1])
            accesible=solution.resolution()
    ...
    def decision(self,position_lab,vue,position_joueur):
        ...
        Entrées:
            ...
            la position du joueur dans le labrinthe
        Sorties:
            La prochaine position voulue par le monstre
        """
        prochaine_position=[]
        
        if self.joueur_accesible(position_lab,vue,position_joueur):
            prochaine_position=self.rush(position_lab,vue,position_joueur)
        else:
            prochaine_position=self.cherche(vue,position_lab)
    ...
    def rush(self,position_lab,vue,position_joueur):
        ...
        Elle prend en entrée:
            ...
            la position du joueur dans le labrinthe
        Sorties:
            la prochaine position voulue
        """
        #on initialise le résolveut pour qu'il nous trouve la prochaine position
        resolveur= Resolveur(vue,len(vue),len(vue[0]),position_joueur[0],position_joueur[1],self.position[0]-position_lab[0],self.position[1]-position_lab[1])
        chemin=resolveur.resolution(True)
        #on renvoie la prochaine action a effectuer
        position_suivante=None
        if chemin!=None:
            if len(chemin)>1:
                position_suivante=chemin[1]
        return position_suivante
